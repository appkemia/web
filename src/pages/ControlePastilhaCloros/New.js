...
const New = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      const response = await api.post('/controle-pastilha-cloros', dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/controle-pastilha-cloros/show/${response.data.id}`);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      ...
      <CardContainer
        ...
        iconColor="blue"
        title="Novo Controle Pastilha Cloro"
        loading={loading}
      >
        <GridContainer>
