...
const UsuarioEdit = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      await api.put(`/controle-concentracao-cloros/${id}`, dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/controle-concentracao-cloros/show/${id}`);
    } catch (error) {
      setLoading(false);
  ...
  useEffect(() => {
    async function getData() {
      ...
      try {
        const response = await api.get(`/controle-concentracao-cloros/${id}`);
        setLoading(false);
        const { data } = response;
    ...
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        <Typography color="textPrimary">
          Editando Controle Concentração Cloro
        </Typography>
        );
      </Breadcrumbs>
      ...
      <CardContainer
        ...
        iconColor="blue"
        title="Editanto Controle Concentração Cloro"
        loading={loading}
      >
        <GridContainer>
