...
const UsuarioEdit = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      await api.put(`/locais/${id}`, dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/locais/show/${id}`);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        <Typography color="textPrimary">Editando Local</Typography>
        );
      </Breadcrumbs>
