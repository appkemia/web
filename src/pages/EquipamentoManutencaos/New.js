...
const New = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      const response = await api.post('/equipamento-manutencaos', dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/equipamento-manutencaos/show/${response.data.id}`);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        />
        <Typography color="textPrimary">Novo Equipamento Manutenção</Typography>
        );
      </Breadcrumbs>
      ...
      <CardContainer
        ...
        iconColor="blue"
        title="Novo Equipamento Manutenção"
        loading={loading}
      >
        <GridContainer>
