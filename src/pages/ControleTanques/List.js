...
const List = () => {
  ...
  async function onDelete() {
    ...
    try {
      await api.delete(`/controle-tanques/${deleteData.id}`, {});
  ...
  return (
    <CardContainer
      ...
      iconColor="blue"
      title="Controle Tanque"
      loading={loading}
    >
      <GridAction>
        <Can I="new" a="ControleTanques">
          <Button
            onClick={() => navigate('/controle-tanques/new')}
            color="blue"
          >
            Novo
          </Button>
      ...
      <Table
        ...
        columns={
          <>
            ...
            <TableCell className={classesTable.cell}>Tanque</TableCell>
          </>
        }
      >
        {data.map((row) => {
          return (
            <TableRow key={row.id} hover className={classesTable.row}>
              <TableCell>
                <Can I="show" a="ControleTanques">
                  <IconButton
                    tooltip="Exibir"
                    onClick={() => navigate(`/controle-tanques/show/${row.id}`)}
                    Icon={RemoveRedEyeIcon}
                    iconColor="green"
                  />
                </Can>
                <Can I="edit" a="ControleTanques">
                  <IconButton
                    tooltip="Editar"
                    onClick={() => navigate(`/controle-tanques/edit/${row.id}`)}
                    Icon={EditIcon}
                    iconColor="orange"
                  />
                </Can>
                <Can I="delete" a="ControleTanques">
                  <IconButton
                    tooltip="Excluir"
                    onClick={() => {
                      setDeleteData(row);
              ...
              <TableCell>{dig(row, 'tanque', 'nome')}</TableCell>
            </TableRow>
          );
        })}
      </Table>
