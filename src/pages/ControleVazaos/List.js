...
const List = () => {
  ...
  async function onDelete() {
    ...
    try {
      await api.delete(`/controle-vazaos/${deleteData.id}`, {});
  ...
  return (
    <CardContainer
      ...
      iconColor="blue"
      title="Controle Vazão"
      loading={loading}
    >
      <GridAction>
        <Can I="new" a="ControleVazaos">
          <Button onClick={() => navigate('/controle-vazaos/new')} color="blue">
            Novo
          </Button>
        </Can>
      </GridAction>
      ...
      <Table
        ...
        columns={
          <>
            ...
            <TableCell className={classesTable.cell}>Vazão Dia</TableCell>
          </>
        }
      >
        {data.map((row) => {
          return (
            <TableRow key={row.id} hover className={classesTable.row}>
              <TableCell>
                <Can I="show" a="ControleVazaos">
                  <IconButton
                    tooltip="Exibir"
                    onClick={() => navigate(`/controle-vazaos/show/${row.id}`)}
                    Icon={RemoveRedEyeIcon}
                    iconColor="green"
                  />
                </Can>
                <Can I="edit" a="ControleVazaos">
                  <IconButton
                    tooltip="Editar"
                    onClick={() => navigate(`/controle-vazaos/edit/${row.id}`)}
                    Icon={EditIcon}
                    iconColor="orange"
                  />
                </Can>
                <Can I="delete" a="ControleVazaos">
                  <IconButton
                    tooltip="Excluir"
                    onClick={() => {
                      setDeleteData(row);
              ...
              <TableCell>{row.vazao_dia}</TableCell>
            </TableRow>
          );
        })}
      </Table>
