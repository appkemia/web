...
const UsuarioEdit = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      await api.put(`/controle-vazaos/${id}`, dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/controle-vazaos/show/${id}`);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        />
        <Typography color="textPrimary">Editando Controle Vazão</Typography>
        );
      </Breadcrumbs>
