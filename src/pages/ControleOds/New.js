...
const New = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      const response = await api.post('/controle-ods', dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/controle-ods/show/${response.data.id}`);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        />
        <Typography color="textPrimary">Novo controle de OD</Typography>
        );
      </Breadcrumbs>
