...
const UsuarioEdit = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      await api.put(`/controle-phs/${id}`, dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/controle-phs/show/${id}`);
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        />
        <Typography color="textPrimary">Editando controle de pH</Typography>
        );
      </Breadcrumbs>
