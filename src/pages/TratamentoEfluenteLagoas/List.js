...
const List = () => {
  ...
  async function onDelete() {
    ...
    try {
      await api.delete(`/tratamento-efluente-lagoas/${deleteData.id}`, {});
  ...
  useEffect(() => {
    async function getData() {
      ...
      try {
        ...
        const response = await api.get(`/tratamento-efluente-lagoas${params}`);
        setLoading(false);
        const { data } = response;
        setData(data);
      } catch (error) {
      ...
      <Table
        ...
        columns={
          <>
            ...
            <TableCell className={classesTable.cell}>Lagoa</TableCell>
          </>
        }
      >
        {data.map((row) => {
          return (
            <TableRow key={row.id} hover className={classesTable.row}>
              ...
              <TableCell>{dig(row, 'lagoa', 'nome')}</TableCell>
            </TableRow>
          );
        })}
      </Table>
