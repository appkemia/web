...
const UsuarioEdit = () => {
  ...
  async function onSubmit(event) {
    ...
    setLoading(true);
    try {
      await api.put(`/tratamento-efluente-lagoas/${id}`, dados);
      setLoading(false);

      toast.success('Salvo com sucesso!');
      navigate(`/tratamento-efluente-lagoas/show/${id}`);
    } catch (error) {
      setLoading(false);
  ...
  useEffect(() => {
    async function getData() {
      ...
      try {
        ...
        setOd(data.od);
        setSs(data.ss);
        setAeracao(data.aeracao);
        setObservacao(data.observacao);
        setNivel_lagoa(data.nivel_lagoa);
        setBomba_recalque_funcionando(data.bomba_recalque_funcionando);
        setObservacao_geral(data.observacao_geral);
        setLagoa(data.lagoa);
    ...
    <>
      <Breadcrumbs maxItems={4} aria-label="breadcrumb">
        ...
        <Typography color="textPrimary">
          Editando Tratamento Efluente Lagoa
        </Typography>
        );
      </Breadcrumbs>
      ...
      <CardContainer
        ...
        iconColor="blue"
        title="Editanto Tratamento Efluente Lagoa"
        loading={loading}
      >
        <GridContainer>
